...
@api_bp.route('/admin/trigger_payment_monitor', methods=['POST'])
def admin_trigger_payment_monitor():
    """管理员API：手动触发资产支付监控任务"""
    try:
        data = request.json
        asset_id = data.get('asset_id')
        ...
        if not asset.payment_tx_hash:
            logger.error(f"资产没有支付交易哈希: {asset_id}")
            return jsonify({
                'success': False,
                'message': f'资产没有支付交易哈希，无法监控支付'
            }), 400
        ...
        try:
            logger.info(f"手动触发支付确认监控任务: AssetID={asset_id}, TxHash={asset.payment_tx_hash}")
            ...
            return jsonify({
                'success': False,
                'message': f'触发监控任务失败: {str(task_error)}',
                'asset_id': asset_id
            }), 500
            
    except Exception as e:
        logger.error(f"管理员API出错: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
