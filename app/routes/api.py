...
def create_trade():
    ...
    try:
        ...
        logger.info(f"创建交易请求: {data}, 钱包地址: {wallet_address}")
        ...
        if asset.remaining_supply < amount:
            return create_error_response('INSUFFICIENT_SUPPLY', f'供应量不足，剩余: {asset.remaining_supply}')
        ...
        trade = Trade(
            ...
            total=Decimal(str(asset.token_price)) * Decimal(str(amount)),
        ...
        logger.info(f"交易记录已创建: trade_id={trade.id}")
        
        # 使用PaymentProcessor准备支付交易
        from app.services.payment_processor import PaymentProcessor
        payment_processor = PaymentProcessor()
        
        payment_result = payment_processor.prepare_asset_purchase_payment(trade.id)
        ...
        db.session.rollback()
        logger.error(f"创建交易失败: {str(e)}", exc_info=True)
...
def confirm_trade():
    ...
    try:
        ...
        logger.info(f"确认交易请求: {data}, 钱包地址: {wallet_address}")
        ...
        if not trade:
            return create_error_response('TRADE_NOT_FOUND', f'交易记录 {trade_id} 不存在')
        
        # 验证钱包地址匹配
        if trade.trader_address.lower() != wallet_address.lower():
            return create_error_response('UNAUTHORIZED', '钱包地址不匹配')
        
        # 使用PaymentProcessor确认支付
        from app.services.payment_processor import PaymentProcessor
        payment_processor = PaymentProcessor()
        
        payment_result = payment_processor.confirm_asset_purchase_payment(trade_id, tx_hash)
        
        if not payment_result.success:
            logger.error(f"确认支付失败: {payment_result.error_message}")
            return create_error_response('PAYMENT_CONFIRMATION_FAILED', payment_result.error_message)
        ...
        db.session.rollback()
        logger.error(f"确认交易失败: {str(e)}", exc_info=True)
        return create_error_response('INTERNAL_SERVER_ERROR', f'确认交易失败: {str(e)}')
