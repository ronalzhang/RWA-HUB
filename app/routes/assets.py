...
def create_asset_page():
    ...
    if not eth_address:
        current_app.logger.warning('访问创建资产页面时未提供钱包地址')
        return jsonify({'error': 'Please connect your wallet first'}), 401
    
    current_app.logger.info(f'用户 {eth_address} 访问创建资产页面')
...
def create_asset():
    ...
    try:
        ...
        # 验证必填字段
        required_fields = ['name', 'asset_type', 'location', 'description', 'total_value', 'token_price', 'annual_revenue']
        for field in required_fields:
            if not request.form.get(field):
                return jsonify({'error': f'Please fill in {field}'}), 400
        ...
        if asset_type == '10' and not request.form.get('area'):
            return jsonify({'error': 'Please fill in building area'}), 400

        # 验证图片
        if 'images[]' not in request.files:
            return jsonify({'error': 'Please upload at least one asset image'}), 400

        # 验证文档
        if 'documents[]' not in request.files:
            return jsonify({'error': 'Please upload required asset documents'}), 400
        ...
        try:
            ...
            # 处理文档
            doc_paths = save_files(
                request.files.getlist('documents[]'),
                f'{asset_type_folder}/{asset.id}/documents',
                ['pdf', 'doc', 'docx'],
                'document'
            )
            if doc_paths:
