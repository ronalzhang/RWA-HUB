...
def check_and_reset_asset():
    ...
    with app.app_context():
        try:
            # 查找资产
            asset = Asset.query.filter_by(token_symbol='RH-203906').first()
            
            if not asset:
                print("❌ 未找到RH-203906资产")
                return False
            ...
            # 4. 确保支付详情正确
            if not asset.payment_details:
                payment_info = {
                    'status': 'confirmed',
                    'confirmed_at': datetime.utcnow().isoformat(),
                    'amount': 100.0,
                    'tx_hash': 'test_payment_hash_for_203906',
...
if __name__ == "__main__":
    print("🔍 检查和重置RH-203906资产状态...")
    success = check_and_reset_asset()
    if success:
        print("\n✅ 重置完成，可以开始测试真实自动上链流程")
    else:
        print("\n❌ 重置失败") 